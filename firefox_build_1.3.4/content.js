...
async function analisarPerfil() {
    ...
    for (let i = 0; i < seedProfiles.length; i++) {
        ...
        console.log(`Digitado: "${profile.username}" - Aguardando resultado...`);
        ...
        while (attempts < maxAttempts && !found) {
            ...
            if (noResultsFound) break;
            
            // Debug: contar elementos visíveis
            const visibleElements = allElements.filter(el => el.offsetParent !== null);
            console.log(`Tentativa ${attempts}: ${visibleElements.length} elementos visíveis`);
            
            // Verificar se encontrou o perfil com múltiplos métodos
            for (let element of allElements) {
                // Método 1: Link direto
                if (element.href && element.href.includes(profile.username) && element.offsetParent !== null) {
                    found = true;
                    console.log(`✅ FOUND via href: @${profile.username} - ${element.href}`);
                    break;
                }
                
                // Método 2: Texto do username (mais flexível)
                if (element.textContent && element.offsetParent !== null) {
                    ...
                    if ((text.includes(username) || text.includes('@' + username)) && 
                        ...
                        found = true;
                        console.log(`✅ FOUND via text: @${profile.username} - "${element.textContent.substring(0, 50)}..."`);
                        break;
                    }
                }
                
                // Método 3: Atributo alt, title, ou aria-label
                const attrs = [element.alt, element.title, element.getAttribute('aria-label')];
                for (let attr of attrs) {
                    if (attr && attr.toLowerCase().includes(profile.username.toLowerCase())) {
                        found = true;
                        console.log(`✅ FOUND via attribute: @${profile.username} - ${attr}`);
                        break;
                    }
                }
