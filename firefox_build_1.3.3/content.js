        ...
        while (attempts < maxAttempts && !found) {
            ...
            for (let element of allElements) {
                ...
                if (element.href && element.href.includes(profile.username) && element.offsetParent !== null) {
                    found = true;
                    console.log(`FOUND via href: @${profile.username}`);
                    break;
                }
                
                // Método 2: Texto do username
                if (element.textContent && 
                    ...
                    !element.textContent.includes('No results found')) {
                    found = true;
                    console.log(`FOUND via text: @${profile.username}`);
                    break;
                }
                
                // Método 3: Atributo alt ou title
                if ((element.alt && element.alt.toLowerCase().includes(profile.username.toLowerCase())) ||
                    (element.title && element.title.toLowerCase().includes(profile.username.toLowerCase()))) {
                    found = true;
                    console.log(`FOUND via alt/title: @${profile.username}`);
                    break;
                }
                
                // Método 4: Buscar em spans e divs específicos
                if (element.tagName === 'SPAN' || element.tagName === 'DIV') {
                    const text = element.textContent || '';
                    if (text.includes('@' + profile.username) || text === profile.username) {
                        found = true;
                        console.log(`FOUND via span/div: @${profile.username}`);
                        break;
                    }
                }
            }
